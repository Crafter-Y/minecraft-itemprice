...
class View
{
    ...
    public function render($action, $layout)
    {
        ...
        $view_path = strtr($lowername, array('.' => DS));

        $viewfile = ROOT . 'views' . DS . $view_path . DS . $this->action . ".thtml";
        if ($this->action[0] == '.') {
            ...
            if ($this->action[0] == '/') {
                $viewfile = ROOT . 'views' . str_replace('/', DS, $this->action) . '.thtml';
            }
        }
    ...
    {
        $this->elementName = $this->getElementName($name); //in case $name gets overwritten
//		extract($this->controller->viewVars);
//		extract($params);
//		extract($this->helperClasses);

        foreach ([$this->controller->viewVars, $params, $this->helperClasses] as $__array) {
...
//		$GLOBALS['__THIS'] = $this;
        ...
        ob_start();
        $trans_name = strtr($name, array('.' => DS));
        if (file_exists(ROOT . 'views' . DS . 'elements' . DS . strtolower($trans_name) . '.thtml')) {
            require ROOT . 'views' . DS . 'elements' . DS . strtolower($trans_name) . '.thtml';
            return ob_get_clean();
        }
        throw new Exception('views: renderElement: cant find Element [' . $name . ']');
    }
