...
class dispatcher
{
    ...
    protected $forbiddenActions = array(
        'dispatch' => 1,
        'render' => 1,
        'renderView' => 1,
        'renderCachedHtml' => 1,
     ...
     */
    public function dispatch($url, $routes = null)
    {

        foreach ($this->forbiddenActions as $k => $v) {
            $this->forbiddenActions[strtolower($k)] = $v;
        }
        ...
        try {
            ...
            $controller_path= strtr($lowername, array('.' => DS));
            // load controller and check if action exists
            require_once LIB . 'controller.php';
            if (file_exists(ROOT . 'controllers' . DS . $controller_path . '_controller.php')) {
                require_once ROOT . 'controllers' . DS . $controller_path . '_controller.php';
            } else {
                if (file_exists(LIB . 'controllers' . DS . $controller_path . '_controller.php')) {
                    require_once LIB . 'controllers' . DS . $controller_path . '_controller.php';
                } else {
            ...
            $classname = ucfirst(strtr($lowername, array('.' => ''))) . 'Controller';
            ...
            if (!is_callable(array(
                        $c,
                        $this->params['action']
                    )) || ($this->params['action'][0] == '_') || isset($this->forbiddenActions[strtolower($this->params['action'])])) {
                if ($c->before404()) {
                    $this->fourohfour();
                    return;
                }
    ...
    protected function constructController($classname)
    {
        $c = new $classname;
        $this->controller = $c;
     ...
     */
    protected function constructParams($url, $routes = null)
    {
        //do we have routes?
...
//					if (($old != '') && ($old.'/' == substr($url, 0, strlen($old.'/')))) {
        ...
        $kataUrl = isset($_GET['kata']) && is_string($_GET['kata']) ? $_GET['kata'] : '';
        unset($_GET['kata']);
        if (!empty($_GET)) {
     ...
     * @return string
     */
    protected function constructBasePath()
    {
    ...
    protected function handleCachedFiles($what, $version)
    {
