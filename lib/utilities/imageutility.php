...
class ImageUtility
{
    ...
    const UPL_INTERNAL = 2; // upload error (aborted, no permission...)
    const UPL_EXTENSION = 3; // error in PHP extension

    private $_allowedExtensions = array('png', 'gif', 'jpg', 'jpeg');
    private $_allowedMimeTypes = array('image/png', 'image/gif', 'image/jpeg');
    ...
    /**
     * check if the file extension is invalid (not one of the allowed extensions)
     * 
     * do not solely use this function to validate uploaded image files, also check the content with isImageInvalid()
     * 
     * @param string $filename the image file name
     * @return bool true=invalid or false=valid
     */
    function isExtensionInvalid($filename)
    {
     ...
     * @return mixed true=success, false=no filename given or extension invalid, UPLOAD_ERR_
     */
    function simpleUploadError($fieldname)
    {

        if (isset($_FILES[$fieldname]['name']) && !$this->isExtensionInvalid($_FILES[$fieldname]['name'])) {
            if (empty($_FILES[$fieldname]['error']) && is_uploaded_file($_FILES[$fieldname]['tmp_name'])) {
                return true;
            }
        }
