...
abstract class Helper {
	...
	public function urlRel($url = null) {
		...
		$preparedUrl = $this->_prepareUrl($url);

		if (empty ($url)) {
			return htmlentities($this->basePath, ENT_QUOTES, 'UTF-8');
		}

        // for urls without protocol (http/https)
        if ($url[0] == '/' && $url[1] == '/') {
            return $preparedUrl;
        }

		if ($preparedUrl[0] == '/') {
			return htmlentities($this->basePath, ENT_QUOTES, 'UTF-8') . substr($preparedUrl, 1);
		}
		...
		return htmlentities($this->basePath, ENT_QUOTES, 'UTF-8') . $preparedUrl;
	}
	...
	public function urlAbs($url = null) {
		...
		if (empty ($url)) {
			return htmlentities($this->base, ENT_QUOTES, 'UTF-8');
		}


		// CND hack start
		$cdnPos = strpos($url, 'cdn');
		if ($cdnPos === 0 || $cdnPos === 1)
		{
			return GF_Config_Cdn::url($url);
		}
		// CDN hack end
		...
		if ($url[0] == '/') {
			return htmlentities($this->base, ENT_QUOTES, 'UTF-8') . substr($preparedUrl, 1);
		}
		...
		return htmlentities($this->base, ENT_QUOTES, 'UTF-8') . $preparedUrl;
	}

    /**
     * Parse the url into parts and escape it
     *
     * @param string $url
     * @return string
     */
	protected function _prepareUrl($url)
	{
		if (!empty($url))
		{
			// workaround //// pathes KAZSHOP-1117
			$shemaAdded = false;
            if ($this->_urlAddSchema && substr($url, 0, 2) == '//')
			{
				$shemaAdded = true;
				$url = 'http:' . $url;
			}
			...
			$scheme   = isset($parsedUrl['scheme']) ? $parsedUrl['scheme'] . '://' :    (substr($url, 0, 2) == '//' ? '//' : '');
			...
			{
				$path = htmlentities($parsedUrl['path'], ENT_QUOTES, 'UTF-8');
			}
			else
			{
				$path = '';
	 ...
	 * @param mixed $exclude null or array of attribute-names not to append (eg. when they are framework-parameters, not html-attributes)
 ...
 */
	function tokenUrl($url) {
		...
		$x = strpos($url,'#');
		if (false !== $x) {
			return substr($url,0,$x).'?__token='.$token.substr($url,$x);
		}
