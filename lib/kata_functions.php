...
class kataFunc
{
     ...
     * @return string
     */
    static function getValueInfo($val)
    {
        if (is_null($val)) {
            return 'null';
        }
    ...
    static function autoloader($classname)
    {
        ...
        switch ($cname) {
            ...
            /*** GF_SPECIFIC ***/
            case substr($classname, 0, 3) == 'GF_' or substr($classname, 0, 5) == 'Zend_' or substr($classname, 0,
                    6) == 'ZendX_' :
            ...
            case substr($cname, -10, 10) == 'controller' :
                $trcname = strtr($cname, array('.' => DS));
                if (file_exists(ROOT . 'controllers' . DS . substr($trcname, 0, -10) . '_controller.php')) {
                    require_once ROOT . 'controllers' . DS . substr($trcname, 0, -10) . '_controller.php';
                } else {
                    require_once ROOT . 'controllers' . DS . 'templates' . DS . substr($trcname, 0,
                            -10) . '_controller.php';
                }
                break;
    ...
    {
        if (DEBUG < 2 || (defined('IS_AJAX') && IS_AJAX == 1)) {
            return;
        }
        if ($isTable) {
