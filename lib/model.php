...
/**
 * @desc The base model-class that all models derive from
 * @package kata_model
...
class Model
{
    ...
    protected function setupDbo($connName)
    {
        ...
        $dbvars[$connName]['connName'] = $connName;
        if (!class_exists($dboname, false)) {
    ...
    /**
     * execute an actual query asynchronous on the database
     *
     * @param string $s the sql to execute
     * @param string $idnames can be used to have the keys of the returned array equal the value ob the column given here (instead of just heaving 0..x as keys)
     * @return mixed returns array with results OR integer with insertid OR integer updated rows OR null
     */
    function queryAsync($s = null, $idnames = false)
    {
        if (empty ($s)) {
            throw new InvalidArgumentException('no query is specified');
        }
        if (!$this->dbo()->isAsyncSupported()) {
            writeLog('Cannot execute queries asynchonous. Query is executed synchronously: ' . $s, KATA_ERROR);
        }
        return $this->dbo()->queryAsync($s, $idnames);
    }

    function isAsyncQueryPending()
    {
        return ($this->dbo()->isAsyncQueryPending());
    }

    function fetchAsyncResult()
    {
        return $this->dbo()->fetchAsyncResult();
    }
    ...
    /**
     * Quotes and escapes parameter $mixed. If $mixed is an array, each array
     * value is quoted seperately and returned as comma separated string.
     *
     * @param unknown $mixed
     * @return string
     */
    function quoteMixed($mixed)
    {
        if (is_array($mixed)) {
            $tmp = array();
            foreach ($mixed as $s) {
